...
async function renderToday(){
 ...
 box.innerHTML=`<div class="cover-wrap"><div class="cover-fallback">${a.artist.slice(0,2).toUpperCase()}</div></div><div class="info"><div class="date">${d.toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"})}${isToday?'<span class="today-label">TODAY</span>':""}</div><h2>${a.title}</h2><div class="artist">${a.artist}</div><div class="rating">★ <b>${a.rating}</b> / 10</div><div class="meta"><div><label>Genre</label><div>${a.genre}</div></div><div><label>Best track</label><div>${a.track}</div></div><div><label>Released</label><div>${a.release}</div></div></div>${a.recommended&&a.recommended!=="—"?`<div class="recommend">Recommended by: <strong>${a.recommended}</strong></div>`:""}</div><aside class="singles"><div class="section-kicker">SINGLES</div><div class="single-list">${(a.singles||[]).map((x,n)=>`<div class="single"><span class="single-number">${String(n+1).padStart(2,"0")}</span><strong>${x}</strong></div>`).join("")}</div><div class="single-meta"><div><label>Tracks</label><strong>${a.tracks}</strong></div><div><label>Length</label><strong>${a.length}</strong></div></div>${a.spotify?`<div class="listen-block"><div class="section-kicker">LISTEN</div><iframe class="spotify-embed" src="${a.spotify}" title="Listen to ${a.title} by ${a.artist} on Spotify" loading="lazy" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"></iframe><a class="spotify-link" href="${a.spotify.replace("/embed/","/")}" target="_blank" rel="noopener">Open in Spotify ↗</a></div>`:""}</aside>`;
 const url=getCover(a);if(url){const wrap=document.querySelector(".cover-wrap");if(wrap){const img=document.createElement("img");img.className="cover";img.alt=`${a.title} by ${a.artist} album cover`;img.src=url;img.onerror=()=>{img.remove();wrap.innerHTML=`<div class="cover-fallback">${a.artist.slice(0,2).toUpperCase()}</div>`;};if(a.spotify){const link=document.createElement("a");link.href=a.spotify.replace("/embed/","/");link.target="_blank";link.rel="noopener";link.className="cover-link";link.title=`Open ${a.title} on Spotify`;link.appendChild(img);wrap.replaceChildren(link)}else{wrap.replaceChildren(img)}}}
}
